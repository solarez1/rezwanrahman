...
aws.exe cloudwatch put-metric-alarm --alarm-name LowMemoryGB --metric-name LowMemoryGB --dimensions "Name=Installation,Value=LowMemory" --namespace "System/Windows" --statistic Maximum --period 300 --threshold 0.05 --comparison-operator LessThanThreshold --evaluation-periods 1 --unit "Gigabytes" --region $region
